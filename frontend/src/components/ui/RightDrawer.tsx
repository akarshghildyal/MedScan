...
export function RightDrawer({
    ...
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                <Dialog.Overlay className="fixed inset-0 z-40 bg-transparent transition-opacity duration-150 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
                <Dialog.Content
                    className={cn(
                        'fixed top-16 bottom-0 right-0 z-50 flex w-full max-w-[640px] flex-col bg-card/95 backdrop-blur-xl shadow-2xl transition-transform duration-200 ease-out focus:outline-none focus:ring-0',
